...
def excel_path():
    ...
    EXCEL_FILE = ''.join(
        file for file in file_list if file.lower().endswith((".xls", ".xlsx")))
    return EXCEL_FILE

def ticket_num(file_name: str):
    FILE_NAME = file_name
    df_credit = pd.read_excel(f"{Constants.ATTACHMENT}\\{FILE_NAME}", index_col=False)
    ticket = df_credit.loc[0, 'Ticket No']
    return ticket

def read_credit_info_report(file_name: str):
    dict = {}
    FILE_NAME = file_name
    df_credit = pd.read_excel(f"{Constants.ATTACHMENT}\\{FILE_NAME}", index_col=False)
...
def read_individual_detail(file_name: str, logger):
    logger.info(f"Ticket from mail: Reading individual detail")
    FILE_NAME = file_name
...
def read_individual_guarantor_detail(file_name: str, logger):
    logger.info(f"Ticket from mail: Read individual gurarantor detail")
    FILE_NAME = file_name
    df = pd.read_excel(f"{Constants.ATTACHMENT}\\{FILE_NAME}", index_col=False)
    ...
    if header == []:
        ...
        header = df.index[df['Ticket No'] == 'Individual Guarantor Details'].tolist()[
            0]
    BuiltIn().log_to_console(f'{header}----------------->')
    ...
    for num in range(20):
        try:
            if df_individual_guarantor.loc[row, 'First Name'] == '' or df_individual_guarantor.loc[row, 'First Name'] == 'Institution Details - 1':
                break
        except:
            break
        ...
        BuiltIn().log_to_console(df_individual_guarantor.loc[row, 'First Name'])
        newdict = dict.copy()
        dictlist.append(newdict)
        row += 1
    return dictlist

def read_institution_detail(file_name: str, logger):
    logger.info(f"Ticket from email: read instituition detail")
    FILE_NAME = file_name
    df = pd.read_excel(f"{Constants.ATTACHMENT}\\{FILE_NAME}", index_col=False)
...
def institution_checker(file_name: str):
...
def read_institution_guarantor_detail(file_name: str, logger):
    logger.info(f"Ticket from email: Read institution gurantor detail")
    FILE_NAME = file_name
    df = pd.read_excel(f"{Constants.ATTACHMENT}\\{FILE_NAME}", index_col=False)
    header = df.index[df['Ticket No'] ==
                      'Institution Guarantor Details'].tolist()
    ...
    for num in range(20):    
        try:
            if df_institution_guarantor.loc[row, 'Name of Company:'] == '' or df_institution_guarantor.loc[row, 'Name of Company:'] == 'Individual Guarantor Details':
                break
        except:
            break
